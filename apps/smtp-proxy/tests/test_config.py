...
class ConfigLoadingTests(unittest.TestCase):
    def test_existing_env_secret_values_keep_working(self):
        with mock.patch.dict(
            os.environ,
            {
                ...
                "SMTP_PROXY_USERNAME": "env-smtp-username",
                "SMTP_PROXY_PASSWORD": "env-smtp-password",
                "UPSTREAM_SMTP_USERNAME": "env-upstream-username",
        ...
        self.assertEqual(config.username, "env-smtp-username")
        self.assertEqual(config.password, "env-smtp-password")
        self.assertEqual(config.upstream_username, "env-upstream-username")
    ...
    def test_secret_file_values_load_into_config_fields(self):
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            smtp_username_file = os.path.join(tempdir, "smtp-proxy-username")
            smtp_password_file = os.path.join(tempdir, "smtp-proxy-password")
            upstream_username_file = os.path.join(tempdir, "upstream-smtp-username")
            upstream_password_file = os.path.join(tempdir, "upstream-smtp-password")
            self._write(api_key_file, "file-api-key\n")
            self._write(smtp_username_file, "file-smtp-username\n")
            self._write(smtp_password_file, "file-smtp-password \n")
            self._write(upstream_username_file, "file-upstream-username\n")
            ...
            with mock.patch.dict(
                os.environ,
                {
                    ...
                    "SMTP_PROXY_USERNAME_FILE": smtp_username_file,
                    "SMTP_PROXY_PASSWORD_FILE": smtp_password_file,
                    "UPSTREAM_SMTP_USERNAME_FILE": upstream_username_file,
        ...
        self.assertEqual(config.username, "file-smtp-username")
        self.assertEqual(config.password, "file-smtp-password ")
        self.assertEqual(config.upstream_username, "file-upstream-username")
    ...
    def test_direct_env_secret_precedence_does_not_read_missing_file(self):
        with mock.patch.dict(
            os.environ,
            {
                ...
                "SMTP_PROXY_USERNAME": "env-smtp-username",
                "SMTP_PROXY_USERNAME_FILE": "/path/that/does/not/exist",
                "UPSTREAM_SMTP_USERNAME": "env-upstream-username",
                "UPSTREAM_SMTP_USERNAME_FILE": "/path/that/does/not/exist",
            },
            clear=True,
        ):
            config = load_config_from_env()
